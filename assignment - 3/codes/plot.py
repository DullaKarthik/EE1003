...
# Highlight the root point
plt.scatter(root, f(root), color="red", zorder=5, label=f"Root Point: ({root:.2f}, {f(root):.2f})")

# Optional: Highlight additional points
highlight_points = [20]  # Example points to highlight
for point in highlight_points:
    plt.scatter(point, f(point), color="green", zorder=5)
    plt.text(point, f(point), f"({point:.2f}, {f(point):.2f})", fontsize=9, color="green")
